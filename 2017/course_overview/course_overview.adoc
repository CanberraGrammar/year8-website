...
Year 9 Web App Development is a one year course that forms part of the Information Technology program at Canberra Grammar School. The course is based upon, and complies with, the Information and Software Technology (IST) syllabus provided by the NSW Board of Studies.
...
{set:cellbgcolor:grey}
2+^.^|
*Topic 7: Finishing Advanced Topics* +
[outcomes]##Outcomes: 5.1.1, 5.1.2, 5.2.1, 5.2.2, 5.2.3, 5.3.1, 5.3.2, 5.4.1, 5.5.1##
{set:cellbgcolor:white}
2+|
{set:cellbgcolor:grey}
...
All students are required to purchase the two textbooks. *Each textbook costs $50 and will be charged to the student's account.* The Semester One textbook will be distributed and charged at the beginning of Term One, and the Semester Two textbook will be distributed and charged at the beginning of Term Two.

As we are using an online integrated development environment (IDE) for writing our web page code, together with hosting all files online, students will have access to the same development software (together with their files) from any computer. These can be accessed through any standards-compliant web browser, although all our work will be completed using the free Google Chrome web browser.

For this course, each student is required to have their own laptop computer running a full desktop operating system (e.g. OS X, Windows, etc.). A tablet (such as a iPad or Surface) is not sufficient or appropriate for this course which involves software development. Note, if you are considering purchasing a new laptop then it would be worthwhile speaking with Mr Purcell to ensure that your laptop will meet your needs into future years — particularly if you are intending to continue and study Year 10 IST which has specific laptop requirements, due to the nature of the course (iOS app development).
