...
You have the choice to work either *individually OR in pairs* for this assignment, although we *highly recommend working individually*.

NOTE: If you work in pairs, the expectation for overall programming quality will be much higher, and marking significantly harsher. There will also be no individual mark for this task, and it is expected that each group member contributes equally towards the assignment.
...
In general, we recommend you create games that have relatively simple interactions with HTML DOM elements. This should be similar in style to the examples we have covered in class, Cookie Clicker and Battleship. These games involve text inputs, buttons and grids.

Creating arcade style games with sprites, players, enemies and obstacles is best with a game engine such as Unity, which you will learn in Year 9.

The idea is to have a game that is not overly complex, but can be enhanced in creative ways to make it as complex as you like.
