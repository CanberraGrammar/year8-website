...
This assignment was last updated on *Thursday 30 March 2017*.
...
You also cannot embed applets (e.g. Java, Flash) into the website - all the work must be in client-side markup (e.g. HTML, HTML5), formatting (e.g. CSS), and/or scripting languages (e.g. JavaScript, jQuery, etc.). The only exception is embedding YouTube videos - browsers such as Chrome will use the HTML5 player, but other browsers (e.g. Internet Explorer, Safari) may use a Flash-based player. In this situation the embedding of the video with the YouTube video is allowed, since it's unavoidable.
...
One person in each team should setup a workspace called *Assignment Two*. This workspace should then be shared with read/write access to the other member of the team, allowing both to work on the single project. Be careful to only share the workspace with members of your own team, and not other students.
...
Once you have formed into a team contact Mr Purcell or Mrs Nagy via email so we can record your team members.
